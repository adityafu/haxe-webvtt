...
var ConverterContext = $hx_exports["ConverterContext"] = function(inputModuleName,moduleSearchPath,compilerOptions,stdLibMap,options) {
	...
	this.haxeTypePathMap = new HaxeTypePathMap(this.packageName != null ? this.packageName : this.normalizedInputModuleName,this.program,this.symbolAccessMap,stdLibMap);
...
ConverterContext.prototype = {
		...
		if(!hxTypePath.isExistingStdLibType) {
			if(Lambda.exists(tool_TsSymbolTools.getDeclarationsArray(symbol),function(d) {
				...
				if(declaredWithinInputModule) {
					...
					this.declarationSymbolQueue.tryEnqueue(symbol);
				} else if(this.queueExternalSymbols) {
			...
			if((symbol.flags & ts.SymbolFlags.Enum) != 0) {
				...
				if(classDeclaration != null) {
					if(!Lambda.exists(fields,function(f) {
						...
						var inlobj_pos_min = 20681;
						var inlobj_pos_max = 20706;
						var inlobj_isExtern = false;
						var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
						var inlobj_fields_0 = { name : "new", kind : haxe_macro_FieldType.FFun({ args : [], ret : null, expr : null, params : []}), pos : { file : "src/ConverterContext.hx", min : 20689, max : 20704}};
		...
		if((symbol.flags & ts.SymbolFlags.Function) != 0) {
			...
			if(tmp) {
				...
				case 0:
					...
					var inlobj_name = "";
					var inlobj_pos_file = "src/ConverterContext.hx";
					var inlobj_pos_min = 30319;
					var inlobj_pos_max = 30411;
					var inlobj_isExtern = false;
					var inlobj_kind = haxe_macro_TypeDefKind.TDClass(null,[],false,false,false);
					var inlobj_fields_0 = { name : "get_value", access : [haxe_macro_Access.AStatic,haxe_macro_Access.AInline], kind : haxe_macro_FieldType.FFun({ args : [], ret : type, expr : { expr : haxe_macro_ExprDef.EReturn({ expr : haxe_macro_ExprDef.ECast({ expr : haxe_macro_ExprDef.EConst(haxe_macro_Constant.CIdent(haxeClassName)), pos : { file : "src/ConverterContext.hx", min : 30388, max : 30403}},null), pos : { file : "src/ConverterContext.hx", min : 30381, max : 30403}}), pos : { file : "src/ConverterContext.hx", min : 30374, max : 30403}}, params : []}), pos : { file : "src/ConverterContext.hx", min : 30333, max : 30403}};
	...
	,complexTypeFromIntersectionType: function(intersectionType,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		} else if(this.allowIntersectionRasterization) {
	...
	,functionFieldFromSignatures: function(fieldName,signatures,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		while(_g < _g1) {
			...
			fun.expr = { expr : haxe_macro_ExprDef.EBlock([]), pos : { file : "src/ConverterContext.hx", min : 53241, max : 53243}};
	...
	,typeParamDeclFromTypeDeclarationSymbol: function(symbol,accessContext,enclosingDeclaration) {
		...
		while(_g1 < _g2.length) {
			var t = _g2[_g1];
			++_g1;
			_g.push({ name : tool_TsSyntaxTools.typeParameterDeclarationName(t), constraints : this.enableTypeParameterConstraints && t.constraint != null ? [this.complexTypeFromTypeNode(t.constraint,symbol,accessContext,enclosingDeclaration)] : null});
		}
		return _g;
	}
	,typeParamDeclFromTsTypeParameter: function(typeParameter,moduleSymbol,accessContext,enclosingDeclaration) {
		...
		var hxConstraint = this.enableTypeParameterConstraints && typeParamNode != null && typeParamNode.constraint != null ? this.complexTypeFromTypeNode(typeParamNode.constraint,moduleSymbol,accessContext,enclosingDeclaration) : null;
...
var HaxeTypePathMap = function(inputParentModuleName,program,symbolAccessMap,stdLibMap) {
...
Main.main = function() {
	...
	var cliOptions = { cwd : null, outputPath : null, tsConfigFilePath : null, tsCompilerOptions : [], moduleNames : [], moduleSearchPath : ".", allDependencies : false, noOutput : false, locationComments : false, libWrapper : true, logLevel : 1, stdLibMode : StdLibMode.DefaultTypeMap, noGlobal : false, noModular : false, allowIntersectionRasterization : false, queueExternalSymbols : false, skipDependencies : false, enableTypeParameterConstraints : false};
	var help = false;
	var noColor = false;
	var silent = false;
	var defaultValueFormatting = "yellow";
	var argHandler = { options : [{ flags : ["--output","-o"], args : [{ name : "path", opt : false, type : "String", value : null}], doc : "Set output directory for generated externs (default <" + defaultValueFormatting + ">\".haxelib\"</>)"},{ flags : ["--moduleSearchPath","-p"], args : [{ name : "path", opt : false, type : "String", value : null}], doc : "Path to use when searching for modules"},{ flags : ["--all"], args : [], doc : "Convert all dependencies referenced in package.json (that have type definitions)"},{ flags : ["--tsconfig"], args : [{ name : "path", opt : false, type : "String", value : null}], doc : "Set path to tsconfig file to use when processing the .d.ts files"},{ flags : ["--target"], args : [{ name : "scriptTarget", opt : false, type : "String", value : null}], doc : "Set ts compiler option `--target`, takes precedent over options provided by --tsconfig (default <" + defaultValueFormatting + ">\"ES6\"</>)"},{ flags : ["--moduleResolution"], args : [{ name : "kind", opt : false, type : "String", value : null}], doc : "Set ts compiler option `--moduleResolution`, takes precedent over options provided by --tsconfig (default <" + defaultValueFormatting + ">\"Node\"</>)"},{ flags : ["--sourceLocation"], args : [], doc : "Enables printing the corresponding source file and line number for each declaration"},{ flags : ["--skipDependencies"], args : [], doc : "Disables converting dependencies of a module (using this may mean the generated haxe references external types that will not be generated)"},{ flags : ["--useSystemHaxe"], args : [], doc : "Use system haxe version when mapping types to the haxe standard library. By default, standard library types for haxe " + Main.defaultStdLibTypeMap.haxeVersion + " are used"},{ flags : ["--constraints"], args : [], doc : "Enables generating type parameter constraints. This will often work fine for many modules but it is a WIP feature"},{ flags : ["--noGlobal"], args : [], doc : "Disables generating externs for types exposed in the global scope (i.e. types accessible via @:native)"},{ flags : ["--noModular"], args : [], doc : "Disables generating externs for types exposed via modules (i.e. types accessible via @:jsRequire)"},{ flags : ["--noStdLib"], args : [], doc : "Disables mapping types to the haxe standard library – this means externs will be generated for built-in types"},{ flags : ["--noLibWrap"], args : [], doc : "Disables wrapping the generated externs in a haxelib-style library. Use this option if you intend to use the externs via a class-path rather than as a library"},{ flags : ["--noOutput"], args : [], doc : "Runs conversion but doesn't save files"},{ flags : ["--noColor"], args : [], doc : "Disable terminal colors"},{ flags : ["--no-color"], args : [], doc : null},{ flags : ["--help"], args : [], doc : "Show this help"},{ flags : ["--silent"], args : [], doc : "Disable command-line output"},{ flags : ["--noWarn"], args : [], doc : "Disable printing warnings"},{ flags : ["--verbose"], args : [], doc : "Print all logs"},{ flags : ["--allowIntersectionRasterization"], args : [], doc : null},{ flags : ["--includeExternal"], args : [], doc : null},{ flags : ["--noDts2hxVersion"], args : [], doc : null}], parse : function(__args) {
		var __index = 0;
	...
	if(userArgs.length == 0) {
		...
		try {
			...
			if(typeof(_g1) == "string") {
				var e = _g1;
				var s = Console.errorPrefix + ("" + Std.string(e));
				var outputStream = 2;
				if(outputStream == null) {
					outputStream = 0;
				}
	...
	case 2:
		try {
			...
			var s = Console.logPrefix + ("" + Std.string("Using standard library of system haxe version <b>" + str + "</>"));
			var outputStream = 0;
			if(outputStream == null) {
				outputStream = 0;
			}
...
Main.convertTsModule = function(moduleName,moduleSearchPath,compilerOptions,stdLibTypeMap,cliOptions) {
	...
	if(!cliOptions.noOutput) {
		...
		while(_g_current < _g_length) {
			...
			var isBuiltIn = haxeModule.tsSymbol != null && tool_TsSymbolTools.isBuiltIn(haxeModule.tsSymbol);
			...
			if(cliOptions.noGlobal) {
				if(!skipModule) {
					if(!isBuiltIn) {
						var _g = haxeModule.tsSymbolAccess;
						if(_g == null) {
							skipModule = false;
			...
			if(cliOptions.noModular) {
				if(!skipModule) {
					if(!isBuiltIn) {
						var _g2 = haxeModule.tsSymbolAccess;
						if(_g2 == null) {
							skipModule = false;
						} else {
							switch(_g2._hx_index) {
							case 0:
								var _g3 = _g2.modulePath;
								var _g4 = _g2.moduleSymbol;
								var _g5 = _g2.symbolChain;
								skipModule = true;
								break;
							case 1:
								var _g6 = _g2.moduleName;
								var _g7 = _g2.sourceFileSymbol;
								var _g8 = _g2.symbolChain;
								skipModule = true;
								break;
							default:
								skipModule = false;
